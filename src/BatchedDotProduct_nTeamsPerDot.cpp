...
batched_dot_product(int nx, int ny, int nrepeat, bool use_lambda)
{
  ...
  if (use_lambda)
  {
    for (int k = 0; k < nrepeat; k++)
    {
      ...
      Kokkos::parallel_for("compute_dot_products_lambda", policy, dot_prod_lambda);
    }
  }
  else // use functor version
  {
    for (int k = 0; k < nrepeat; k++)
    {
      // Do batched dot product
      // using hierarchical parallelism, multiple teams per dot-product
      Kokkos::parallel_for("compute_dot_products_functor", policy, dot_prod_functor);
