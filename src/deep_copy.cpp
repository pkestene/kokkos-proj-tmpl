...
// ===============================================================
void
test_deep_copy_functor(int length, int nrepeat)
{
  ...
  // Initialize arrays
  Kokkos::parallel_for(
    length, KOKKOS_LAMBDA(const size_t & i) {
      x(i) = 1;
      y(i) = 1;
    });
  ...
  for (int k = 0; k < nrepeat; k++)
  {
    ...
    // Do compute
    Kokkos::parallel_for(
      length, KOKKOS_LAMBDA(const size_t & i) { y(i) = x(i); });
  }
  timer.stop();
  ...
  printf(" %13i %8lf %20.3e  %10.2f %10.2f\n",
         ...
         1.0e-6 * length * 2 * sizeof(DataType),
         1.0e-9 * length * 2 * nrepeat * sizeof(DataType) / time_seconds);
...
// ===============================================================
void
test_deep_copy_api(int length, int nrepeat)
{
