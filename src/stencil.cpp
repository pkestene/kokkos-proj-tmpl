...
KOKKOS_INLINE_FUNCTION
int
INDEX(int i, int j, int k, int Nx, int Ny, int Nz)
{
  UNUSED(Nx);
  UNUSED(Nz);
#ifdef KOKKOS_ENABLE_CUDA
...
KOKKOS_INLINE_FUNCTION
int
INDEX(int i, int j, int Nx, int Ny)
{
  UNUSED(Nx);
#ifdef KOKKOS_ENABLE_CUDA
...
KOKKOS_INLINE_FUNCTION
int
RINDEX(int i, int j, int k, int Nx, int Ny, int Nz)
{
  UNUSED(Nx);
  UNUSED(Nz);
  return k + Nz * j + Nz * Ny * i; // right layout
...
KOKKOS_INLINE_FUNCTION
void
index2coord_right(int index, int & i, int & j, int & k, int Nx, int Ny, int Nz)
{
  UNUSED(Nx);
  UNUSED(Nz);
  int NyNz = Ny * Nz;
 ...
 */
double
test_stencil_3d_flat(int n, int nrepeat)
{
  ...
  // Initialize arrays
  Kokkos::parallel_for(
    nbCells, KOKKOS_LAMBDA(const int & index) {
      int i, j, k;
    ...
    Kokkos::parallel_for(
      nbCells, KOKKOS_LAMBDA(const int & index) {
        ...
        if (i > 0 and i < n - 1 and j > 0 and j < n - 1 and k > 0 and k < n - 1)
          y(i, j, k) = -5 * x(i, j, k) + (x(i - 1, j, k) + x(i + 1, j, k) + x(i, j - 1, k) +
 ...
 */
double
test_stencil_3d_flat_1d_array(int n, int nrepeat)
{
  ...
  for (int irepeat = 0; irepeat < nrepeat; irepeat++)
  {
    ...
    // Do stencil
    Kokkos::parallel_for(
      nbCellsXY, KOKKOS_LAMBDA(const int & index) {
        int i, j;
 ...
 */
double
test_stencil_3d_flat_vector(int n, int nrepeat, bool use_1d_views)
{
  ...
  if (use_1d_views)
  {
    ...
    for (int irepeat = 0; irepeat < nrepeat; irepeat++)
    {
      ...
      // Do stencil
      Kokkos::parallel_for(
        nbIter, KOKKOS_LAMBDA(const int & index) {
          int i, j;
...
#  pragma ivdep
            ...
            for (int k = 1; k < n - 1; ++k)
            {
              ...
              // 	x(i,j-1,k) + x(i,j+1,k) +
              // 	x(i,j,k-1) + x(i,j,k+1) );
              y_i_j(k) = -5 * x_i_j(k) + (x_im1_j(k) + x_ip1_j(k) + x_i_jm1(k) + x_i_jp1(k) +
                                          x_i_j(k - 1) + x_i_j(k + 1));
            }
          }
        });
 ...
 */
double
test_stencil_3d_range(int n, int nrepeat)
{
  ...
  // Initialize arrays
  Kokkos::parallel_for(
    "init", range, KOKKOS_LAMBDA(const int & i, const int & j, const int & k) {
      x(i, j, k) = init_x(i, j, k);
      y(i, j, k) = init_y(i, j, k);
    });
 ...
 */
double
test_stencil_3d_range_vector(int n, int nrepeat)
{
  ...
  // Initialize arrays
  Kokkos::parallel_for(
    "init", range3d, KOKKOS_LAMBDA(const int & i, const int & j, const int & k) {
      x(i, j, k) = init_x(i, j, k);
      y(i, j, k) = init_y(i, j, k);
    });
  ...
  for (int irepeat = 0; irepeat < nrepeat; irepeat++)
  {
    ...
    // Do stencil
    Kokkos::parallel_for(
      "stencil compute - 3d range vector", range2d, KOKKOS_LAMBDA(const int & i, const int & j) {
        auto x_i_j = Kokkos::subview(x, i, j, Kokkos::ALL());
        auto y_i_j = Kokkos::subview(y, i, j, Kokkos::ALL());
 ...
 * - a ThreadVectorRange Kokkos::policy for the inner loop (for CPU vectorization or GPU warp
 * parallelism)
 *
 ...
 */
double
test_stencil_3d_range_hierarchical(int n, int nrepeat)
{
  ...
  for (int irepeat = 0; irepeat < nrepeat; irepeat++)
  {
    ...
    Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const thread_t & thread) {
        ...
        Kokkos::parallel_for(Kokkos::TeamThreadRange(thread, 1, n - 1), [=](const int & j) {
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(thread, 1, n - 1), [=](const int & k) {
            if (i > 0 and i < n - 1 and j > 0 and j < n - 1)
            {
 ...
 */
double
test_stencil_3d_range_hierarchical2(int n, int nbTeams, int nrepeat)
{
  ...
  for (int irepeat = 0; irepeat < nrepeat; irepeat++)
  {
    ...
    Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const thread_t & thread) {
        ...
        Kokkos::parallel_for(
          Kokkos::TeamThreadRange(thread, chunk_size_per_team), [=](const int & index) {
 ...
 */
double
test_stencil_3d_range_hierarchical3(int n, int nrepeat)
{
  ...
  // Initialize arrays using a 3d range policy
  Kokkos::parallel_for(
    "init", n * n * n, KOKKOS_LAMBDA(const int & index) {
      int i, j, k;
        ...
        Kokkos::parallel_for(Kokkos::TeamThreadRange(thread, 1, n - 1), [=](const int & j) {
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(thread, 1, n - 1), [=](const int & k) {
            if (i > 0 and i < n - 1 and j > 0 and j < n - 1)
            {
              ...
              y(RINDEX(i, j, k, n, n, n)) =
                -5 * x(RINDEX(i, j, k, n, n, n)) +
...
// ===============================================================
void
bench(int nrepeat, bench_type bt)
{

  constexpr int nbTests = 8;

  auto select_sizes = [](bench_type bt) {
    switch (bt)
    {
      ...
      case bench_type::LARGE:
        return std::vector<int>{ 32,  48,  64,  92,  128, 160, 192, 224,
                                 256, 320, 384, 448, 512, 640, 768 };
      default:
        return std::vector<int>{ 32, 48 };
    }
  };
  ...
  std::vector<std::string> test_names = { "# test_stencil_3d_flat",
                                          "# test_stencil_3d_flat_1d_array",
...
main(int argc, char * argv[])
{
  ...
  else
  {
    ...
    std::cout << "========================================\n";
    std::cout << "reference naive test using 3d range and vectorization with team policy - nbTeams "
                 "configurable\n";
