...
public:
  ...
  DotProdFunctor(Kokkos::View<double **, Kokkos::LayoutLeft> x,
  ...
  void
  operator()(const member_t & member) const
  {
    ...
    Kokkos::parallel_reduce(
      ...
      [&](const int & i, double & update) { update += m_x(i, j) * m_y(i, j); },
      dot_prod);
  ...
  run()
  {
    ...
    const int team_size_max =
      DotProdFunctor::team_policy_t(m_ny, 1).team_size_max(*this, Kokkos::ParallelForTag());

    // const team_policy_t policy(m_ny, team_size_max);
    const team_policy_t policy(m_ny, Kokkos::AUTO(), Kokkos::AUTO());

    Kokkos::parallel_for("compute_dot_products_functor", policy, *this);
  }
 ...
 */
void
batched_dot_product(int nx, int ny, int nrepeat, bool use_lambda)
{
  ...
  const int team_size_max =
    team_policy_t(ny, 1).team_size_max(dot_prod_functor, Kokkos::ParallelForTag());
  ...
  else // use functor version
  {
    for (int k = 0; k < nrepeat; k++)
    {
      // Do batched dot product
      // using hierarchical parallelism, one team per dot-product
      Kokkos::parallel_for("compute_dot_products_functor", policy_functor, dot_prod_functor);
